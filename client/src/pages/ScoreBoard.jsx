...
const ScoreBoard = () => {
    ...
    return (
        <div className='flex flex-col min-h-screen bg-gray-50 '>
            <Nav/>
            <div className='flex flex-col h-[95vh] bg-[var(--primary-colour)] text-[var(--primary-colour)]'> 
                <div className='flex flex-wrap justify-center items-center  h-[95vh] gap-x-6'>
                        {session?.participants?.map((participant)=>{
                            let points = 0
                            
                            participant.points.map((p)=>{points += p.points})
                            return <div key={participant._id} className="flex flex-col items-center w-full sm:w-[45%] md:w-[20%] lg:w-[20%] bg-[var(--secondary-colour)] p-4 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300">
